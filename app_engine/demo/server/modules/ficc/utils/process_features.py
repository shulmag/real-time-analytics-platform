...
from modules.ficc.utils.auxiliary_variables import COUPON_FREQUENCY_DICT
from modules.ficc.utils.diff_in_days import diff_in_days
from modules.ficc.utils.days_in_interest_payment import days_in_interest_payment
from modules.ficc.utils.fill_missing_values import fill_missing_values
from modules.ficc.utils.auxiliary_functions import calculate_a_over_e

## Please comment before deploying
# from utils.auxiliary_variables import COUPON_FREQUENCY_DICT
# from utils.diff_in_days import diff_in_days
# from utils.days_in_interest_payment import days_in_interest_payment
# from utils.fill_missing_values import fill_missing_values
# from utils.auxiliary_functions import calculate_a_over_e

def process_features(df):
    df['is_non_transaction_based_compensation'] = False
    
    # quantity of the most recent trade in history
    ...
    # Processing
    for feature in ['par_traded', 'maturity_amount', 'orig_principal_amount', 'issue_amount', 'max_amount_outstanding', 'coupon']:
        df[feature].replace(pd.NA, np.nan, inplace=True)
    ...
    df.issue_amount.replace(-np.inf, np.nan, inplace=True)
    df.maturity_amount.replace(-np.inf, np.nan, inplace=True)
    df.orig_principal_amount.replace(-np.inf, np.nan, inplace=True)
    df.max_amount_outstanding.replace(-np.inf, np.nan, inplace=True)
