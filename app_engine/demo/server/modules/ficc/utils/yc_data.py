...
START_TIME = '09:30'
START_DATE_TIME = EASTERN.localize(datetime.strptime(START_DATE + ':' + START_TIME, YEAR_MONTH_DAY_HOUR_MIN))    # date we began to collect YC data
...
def get_business_day(date_and_time):
    ...
    if not BUSINESS_DAY.is_on_offset(date_and_time):    # `is_on_offset(...)` checks if the date_and_time is a `BUSINESS_DAY`
...
def get_last_business_time(date_and_time):
    ...
    if not is_time_between(MARKET_OPEN, MARKET_CLOSE, date_and_time.time()):    # before `MARKET_OPEN` or after `MARKET_CLOSE`
        date_and_time = date_and_time.replace(hour=MARKET_CLOSE.hour, minute=MARKET_CLOSE.minute)
    return date_and_time
...
def find_last_minute(date_and_time):
    ...
    def iterate_backward(date_and_time, num_mins_back, max_num_times=-1):
        ...
        while not date_and_time_exists_in_redis(date_and_time):    # find the `date_and_time` that is at most `num_mins_back` minutes in the past
            if max_num_times == 0: return date_and_time, False
            if is_time_between(MARKET_OPEN, MARKET_CLOSE, date_and_time.time()):    # only in between 9:30 am to 4pm EST
    ...
    def iterate_forward(date_and_time, num_mins_forward):
        ...
        while date_and_time_exists_in_redis(date_and_time):    # after finding `date_and_time` to most `num_mins_back` minutes in the past, increment up by 1 minute until we cannot find it anymore
    ...
    if date_and_time.replace(tzinfo=EASTERN) < START_DATE_TIME:
        ...
        return iterate_backward_backward_forward(date_and_time, 5)    # iterate_backward(date_and_time, 1)[0]
