...
FEATURES_AND_DEFAULT_COLUMNS = {'days_to_par': 'days_to_maturity',     # intuition: for non-callable bonds, the duration is the time to maturity (previously, we filled `days_to_par` and `days_to_call` with a 0 but we should treat the `days_to_call` for a non-callable bond as the `days_to_maturity` so it is consistent between callable and non-callable bonds)
                                'days_to_call': 'days_to_maturity'}     # intuition: for non-callable bonds, the duration is the time to maturity (previously, we filled `days_to_par` and `days_to_call` with a 0 but we should treat the `days_to_call` for a non-callable bond as the `days_to_maturity` so it is consistent between callable and non-callable bonds)
