...
from auxiliary_variables import USERNAME, \
                                PASSWORD, \
                                QUANTITY, \
                                TRADE_TYPE, \
                                ...
                                LoggedOutError, \
                                CUSIP_ERROR_MESSAGE, \
                                ACCEPTABLE_DISPLAY_TYPES_FOR_YIELD_CURVE, \
                                YEAR_MONTH_DAY_HOUR_MIN_SEC, \
                                NUM_CHARS_IN_YEAR_MONTH_DAY, \
                                NUM_CHARS_IN_HOUR_MIN_SEC, \
                                TOLERANCE

def handle_nan(x):
    """Method to normalize error values returned by batch pricing. Batch pricing now returns None instead of -1 for ytw and price in the event of an error,
    but calling float() on None leads to errors. This method returns None if the ytw or price is missing, Mone, or null. Otherwise, it returns the float of the value."""
    # Treat common empties as None
    if x in (None, '', 'null'):
        return None
    if pd.isna(x):            # catches np.nan, pd.NA, NaT
        return None
    try:
        v = float(x)          # also handles numeric strings like "12.3"
...
def run_tests_with_and_without_file(testing_function: callable, check_if_content_is_equal: bool = False, tolerance: float = TOLERANCE, columns_to_ignore: set[int] = set()):
    '''Runs `testing_function` where the post request is made with and without a file. Assumes that `testing_function` is a 
    single argument function where the boolean argument determines whether the post request is made with or without a 
    file. `check_if_content_is_equal` is a boolean flag that determines whether the content should be checked to be equal 
    as a final step after the calls to `testing_function`.'''
    content_with_file = testing_function(True)
    content_without_file = testing_function(False)
    if check_if_content_is_equal:
        assert lists_are_equal(content_with_file, content_without_file, True, tolerance, columns_to_ignore), f'Output of the post request with file does not match the output of the post request without file. Output with file: \n{content_with_file}\nOutput without file: \n{content_without_file}'
...
@run_multiple_times_if_logged_out
def response_from_batch_pricing(filename, cusip_list=None, quantity_list=None, trade_type_list=None, quantity=QUANTITY, trade_type=TRADE_TYPE, return_execution_time=False, create_file: bool = True):
    ...
    if quantity_list is not None and trade_type_list is None: trade_type_list = [trade_type] * len(cusip_list)    # fill in `trade_type_list` if `cusip_list` and `quantity_list` are already filled out
    data = {'username': USERNAME, 'password': PASSWORD, 'amount': quantity, 'tradeType': trade_type, 'download': True}    # setting `download` to `True` so that the data is returned as a CSV file which is the assumed download format for the automated test; `amount` only fills in values that are empty
    if create_file:
        ...
        data = data | {'cusipList': cusip_list, 'quantityList': quantity_list, 'tradeTypeList': trade_type_list}
        file = dict()
...
def split_ignoring_quotes(input_string: str, delimiter: str = ',') -> list:
    '''Split a string at the `delimter`, but ignore anything inside quotes. Mostly generated by ChatGPT.
    
    >>> split_ignoring_quotes('value1,"value2, still part of value2",value3,"value4, part of, value4"')
    ['value1', 'value2, still part of value2', 'value3', 'value4, part of, value4']
    '''
    input_string_as_StringIO_object = StringIO(input_string)    # convert the input string into a StringIO object
    reader = csv.reader(input_string_as_StringIO_object, delimiter=delimiter)    # use the csv.reader to parse the string
    return next(reader)    # return the first (and only) parsed row as a list


def get_spreadsheet_as_list(request_obj, header_columns: list = FEATURES_FOR_OUTPUT_CSV, spreadsheet_returned_as_json_string: bool = False) -> list:
    ...
    if spreadsheet_returned_as_json_string:
        df = pd.read_json(StringIO(request_obj.json()))    # wrap the JSON string with StringIO to process it as a file-like object, aligning with the updated pandas API; this avoids `FutureWarning: Passing literal json to 'read_json' is deprecated and will be removed in a future version. To read from a literal string, wrap it in a 'StringIO' object`
        ...
        content = [split_ignoring_quotes(row) for row in content]    # ignore quotes because the error messages are in the quotes and error messages may contain commas; see the values in `app_engine/demo/server/modules/exclusions.py::CUSIP_ERROR_MESSAGE` for examples
...
def check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, check='output', reason_for_dollar_price_model=None, error_code=None, post_request_made_with_file: bool = True):
    ...
    if check in ('failure', 'dollar_price_model_used'):
        error_message_idx = FEATURES_FOR_OUTPUT_CSV.index('error_message')
    ...
    for idx, row in enumerate(content):
        cusip = row[cusip_idx]
        if check in ('success', 'failure', 'price_only', 'dollar_price_model_used'):
            new_price = handle_nan(row[price_idx])
            new_ytw = handle_nan(row[ytw_idx])

        if check in ('failure', 'dollar_price_model_used'):
            error_message = row[error_message_idx]
        ...
        if check in ('success', 'price_only'): assert new_price != NUMERICAL_ERROR, f'For CUSIP {cusip_list[idx]}, the predicted price {new_price} should not be equal to {NUMERICAL_ERROR}'    # check that the price is not the value used for numerical error (indicating that an actual price was provided)
        if check == 'success':assert new_ytw != NUMERICAL_ERROR, f'For CUSIP {cusip_list[idx]}, the predicted ytw {new_ytw} should not be equal to {NUMERICAL_ERROR}'    # check that the ytw is not the value used for numerical error (indicating that an actual price was provided)

        if check == 'failure':

            assert new_price == NUMERICAL_ERROR, f'For CUSIP {cusip_list[idx]}, the predicted price {new_price} should be equal to {NUMERICAL_ERROR}'    # check that the price is not the value used for numerical error (indicating that an actual price was provided)
            
            assert new_ytw == NUMERICAL_ERROR, f'For CUSIP {cusip_list[idx]}, the predicted ytw {new_ytw} should be equal to {NUMERICAL_ERROR}'    # check that the ytw is not the value used for numerical error (indicating that an actual price was provided)
            if error_code is not None:
                ...
                assert error_message == CUSIP_ERROR_MESSAGE[error_code], f'CUSIP {cusip} should have had an error of {CUSIP_ERROR_MESSAGE[error_code]}, but instead had an error of {error_message}'

        if check == 'dollar_price_model_used':
            check_if_string_value_can_be_represented_as_a_number(new_price, 'price')
            assert new_price != NUMERICAL_ERROR, f'For CUSIP {cusip_list[idx]}, the predicted price {new_price} should not be equal to {NUMERICAL_ERROR}'    # check that the price is not the value used for numerical error (indicating that an actual price was provided)
            if reason_for_dollar_price_model is not None:
                assert error_message == reason_for_dollar_price_model, f'Error message value should be {reason_for_dollar_price_model}, but was instead {error_message}'
            else:
                assert pd.isna(error_message), f'Error message value should be empty but is instead {error_message}'    # check_if_string_value_cannot_be_represented_as_a_number(ytw, 'YTW')    # TODO: fix this
...
def check_that_batch_pricing_gives_price_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, post_request_made_with_file: bool = True):
    ...
    return check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'success', post_request_made_with_file=post_request_made_with_file)
...
def check_that_batch_pricing_gives_price_but_maybe_not_yield_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, post_request_made_with_file: bool = True):
    ...
    return check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'price_only', post_request_made_with_file=post_request_made_with_file)
...
def check_that_batch_pricing_gives_price_and_dollar_price_used_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, reason_for_dollar_price_model=None, post_request_made_with_file: bool = True):
    '''Check that each CUSIP in `cusip_list` has a valid price and ytw in the request_obj.'''
    return check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'dollar_price_model_used', reason_for_dollar_price_model, post_request_made_with_file=post_request_made_with_file)
...
def check_that_batch_pricing_gives_error_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, error_code=None, post_request_made_with_file: bool = True):
    ...
    return check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'failure', error_code=error_code, post_request_made_with_file=post_request_made_with_file)
...
def lists_are_equal(list1, list2, convert_to_string: bool = False, tolerance: float = TOLERANCE, columns_to_ignore: set[int] = set()):
    '''Ensure that `list1` is equal to `list2`. `list1` and `list2` may be nested lists. `convert_to_string` is a boolean flag that determines 
    whether a non-list item should be converted to a string before comparison. `columns_to_ignore` is a set of column indices that should be ignored.
    
    >>> lists_are_equal([1, 2, 3], [1, 2, 3])    # both lists are identical
    True
    >>> lists_are_equal([1, 2, 3], [1, 2, '3'], convert_to_string=True)    # the last elements are considered equal after conversion
    True
    >>> lists_are_equal([1, 2, 3], [1, 2, '3'])    # the last elements are different because string conversion is not enabled
    False
    >>> lists_are_equal([[1, 2], [3, 4]], [[1, 2], [3, 4]])    # nested lists are identical
    True
    >>> lists_are_equal([[1, 2], [3, 4]], [[1, 2], [3, '4']], convert_to_string=True)    # the last elements in the nested lists match after conversion
    True
    >>> lists_are_equal([[1, 2], [3, 4]], [[1, 2], [3, 5]])    # nested lists are different
    False
    >>> lists_are_equal([1, 2, None], [1, 2, None])    # contain `None` in the same positions
    True
    >>> lists_are_equal([1, 2, None], [1, 2, ''], convert_to_string=True)    # last elements in the nested lists match after conversion
    True
    >>> lists_are_equal([1, 2, 3], [1, 2])    # the lists have different lengths
    False
    >>> lists_are_equal([1, 2, 3], [1, 2, 3, 4])    # the lists have different lengths
    False
    >>> lists_are_equal([1, 2.0], [1, '2'], convert_to_string=True)
    True
    >>> lists_are_equal([pd.NaT], [''], convert_to_string=True)    # `pd.NaT` is treated as an empty string when `convert_to_string` is `True`
    True
    '''
    if isinstance(list1, list) and isinstance(list2, list):
        if len(list1) != len(list2): return False
        for idx, (item1, item2) in enumerate(zip(list1, list2)):
            if isinstance(item1, list) and isinstance(item2, list):    # items are lists
                if not lists_are_equal(item1, item2, convert_to_string, tolerance, columns_to_ignore):    # if items are lists then do not ignore any indices since these indices do not correspond to columns
                    return False
            else:
                if idx not in columns_to_ignore and not lists_are_equal(item1, item2, convert_to_string, tolerance):    # when making the recursive call, do not pass in `columns_to_ignore` since this is only used at the top level
                    return False
        return True
    else:
        if isinstance(list1, str) and isinstance(list2, str):    # attempt to convert to datetime so that there can be a comparison with thresholding
            num_chars_in_datetime = NUM_CHARS_IN_YEAR_MONTH_DAY + NUM_CHARS_IN_HOUR_MIN_SEC + 1    # add 1 for the space in between the year month day and hour min sec; used to truncate the timezone of 'ET' off of the string representation of the datetime
            try:
                list1, list2 = datetime.strptime(list1[:num_chars_in_datetime], YEAR_MONTH_DAY_HOUR_MIN_SEC), datetime.strptime(list2[:num_chars_in_datetime], YEAR_MONTH_DAY_HOUR_MIN_SEC)    # check if the string can be converted to a datetime
                if abs(list1 - list2) <= timedelta(minutes=10): list1 = list2    # within a tolerance of 10 mins is acceptable since when doing realtime pricing there is some gap between the two API calls (especially if the realtime pricing is part of a post request that has point-in-time pricing as well)
            except ValueError:    # string objects cannot be converted to datetimes
                pass
        if convert_to_string and type(list1) != type(list2):
            if pd.isna(list1): list1 = ''
            if pd.isna(list2): list2 = ''
            try:
                list1, list2 = float(list1), float(list2)    # assume that one of them items is a string and the other one is numerical (otherwise raise error)
                if np.isclose(list1, list2, atol=tolerance): list1 = list2    # within a tolerance, since yield may change slight in the window of time it takes between the two calls
            except ValueError:    # possible reason that list1 was unable to be converted to float: (1) list1 = '', (2) list1 is a datetime
                pass
            list1, list2 = str(list1), str(list2)
        return list1 == list2

def load_cusips_from_gcs(bond_type):
    #This function loads cusips from a bucket and returns the a cusip list. The bucket is periodically (currently on the 1st of every month) populated by another cloud function pick_cusips_and_save_to_gcs. The parameter bond_type takes a string.
    #Possible parameter values: "short_maturity", (more will be added).

    storage_client = storage.Client()
    bucket = storage_client.bucket("cusip_pool_for_testing")
    blob = bucket.blob(f"{bond_type}.csv")
    ...
    cusip_list = [row["cusip"] for row in reader if row.get("cusip")]
