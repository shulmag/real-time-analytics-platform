# Modificatoin: Moved loading of data from big query to redis
...
#Finnhub.io API key from Myles's account
finnhub_apikey = 'c516kliad3if5950qksg'
...
def get_data_from_redis():
    ...
    redis_data = pickle.loads(redis_client.get('current_yield_data'))
    return redis_data

def main(request):
    '''
    This is the main function. It takes as input the maturity for which the realtime ytw is needed.
    ...
    #We store the maturity in a numpy array so that transformations can be done 
    t = np.array(t)
    ...
    index_data = redis_data['sp_index_data']
    indices = list(index_data.keys())
    
    #Load daily ETF data 
    # etf_data = load_daily_etf_prices_bq()
    etf_data = redis_data['daily_etf_data']

    #Load maturity data and scalers for those maturities 
    # maturity_df = load_maturity_bq()
    maturity_df = redis_data['sp_index_maturity_data']
    scaler_daily_parameters = redis_data['scalar_coefficient']
    ...
    for current_index in list(best_funds.keys()): 
        #Load the optimal regularization penalty and features for each fund
        current_best_lambda = best_lambdas[current_index]
        current_best_funds = best_funds[current_index]

        #Load the data for the current day and preprocess it ahead of fitting the lasso
        ...
        assert len(model_data) == window_size #check the window size is correct
    ...
    yield_curve_df['Weighted_Maturity'] = yield_curve_df.index.map(maturity_dict).astype(float) #map maturities to each index
    ...
    predictions = ridge.predict(X_curve)[0]
    
    #Return the prediction as json
    return {'status':'Success', 'error':'', 'result':predictions, 'timestamp':target_time}