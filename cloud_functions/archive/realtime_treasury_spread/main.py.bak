'''Retrieving the treasury spread every minute.'''
import pandas as pd
...
def uploadData(df, table_name):
    ...
    job = client.load_table_from_dataframe(
        df, f"eng-reactor-287421.treasury_yield.{table_name}", job_config=job_config
    )
...
def main(args):
    ...
    for ticker in tickers:
        ...
        temp_data.loc[:, 'Date'] = pd.to_datetime(
            temp_data.Date.dt.strftime('%Y-%m-%d %H:%M')
        )
