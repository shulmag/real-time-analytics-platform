# Modificatoin: Modifications in the main function to make code more DRY and reduce complexity


# The majority of intution behind Nelson-Siegel function were found in the following paper
# https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2054689
...
REDIS_CLIENT = redis.Redis(host='10.146.62.92', port=6379, db=0) #The redis server that host the data for yield curve models
...
def load_model_parameters(target_date):
    ...
    memory store 
    '''
    temp_dict = pickle.loads(REDIS_CLIENT.get(target_date))
...
def get_scaled_features(t:np.array, exponential_mean:float, exponential_std:float, laguerre_mean:float, laguerre_std:float):
    ...
    X1 = ((tau*(1-np.exp(-t/tau))/t) - exponential_mean)/exponential_std 
    X2 = (((tau*(1-np.exp(-t/tau))/t) -np.exp(-t/tau)) - laguerre_mean)/laguerre_std 
    return X1, X2
...
def main(request):
    ...
    if request.args:
        ...
        target_date = request.get_json(silent=True).get('target_date')
    else:
        return  {'status':'Failed', 'error':'Error passing arguments', 'result':np.nan}
    ...
    try:
        ...
        return {'status':'Failed', 'error':'Failed to load data from memory store', 'result':np.nan} 
    ...
    prediction = predict_ytw(maturities, const, exponential, laguerre, exponential_mean, exponential_std, laguerre_mean, laguerre_std)
    
    return {'status':'Success', 'error':error, 'result':prediction, 'maturities':maturities}