...
from auxiliary_variables import USERNAME, PASSWORD, QUANTITY, TRADE_TYPE, FEATURES_FOR_OUTPUT_CSV, NUMERICAL_ERROR, REQUEST_URL, DIRECTORY, DOLLAR_PRICE_MODEL_DISPLAY_TEXT, LOGGED_OUT_MESSAGE, LoggedOutError, CUSIP_ERROR_MESSAGE
...
@run_multiple_times_if_logged_out
def response_from_batch_pricing(filename, cusip_list=None, quantity_list=None, trade_type_list=None, trade_type=TRADE_TYPE, return_execution_time=False):
    ...
    is provided, then default to `trade_type`.'''
...
def get_spreadsheet_as_list(request_obj):
    ...
    header = content[0]
    header = header.split(',')
    assert FEATURES_FOR_OUTPUT_CSV == header, f'The header of the spreadsheet should be {FEATURES_FOR_OUTPUT_CSV} but is instead {content}'    # column names in predictions csv are correct

    content = content[1:-1]    # first row is the column names, last row is an empty line
    return [row.split(',') for row in content]


def check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, check='output', reason_for_dollar_price_model=None, error_code=None):
    ...
    error message corresponds to that of `error_code`.'''
...
def check_that_batch_pricing_gives_price_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True):
    ...
    check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'success')


def check_that_batch_pricing_gives_price_but_maybe_not_yield_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True):
    '''Check that each CUSIP in `cusip_list` has a valid price in the request_obj. No check is made on the ytw.'''
    check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'price_only')


def check_that_batch_pricing_gives_price_and_dollar_price_used_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, reason_for_dollar_price_model=None):
    '''Check that each CUSIP in `cusip_list` has a valid price and ytw in the request_obj.'''
    check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'dollar_price_model_used', reason_for_dollar_price_model)


def check_that_batch_pricing_gives_error_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default=True, error_code=None):
    '''Check that each CUSIP in `cusip_list` has some error outputs in the request_obj.'''
    check_that_batch_pricing_gives_output_for_all_cusips(request_obj, cusip_list, check_quantity_equals_default, 'failure', error_code=error_code)
...
def response_from_similar_bonds(**kwargs):
    ...
    default_values = {'minCoupon': 0, 
                      ...
                      'minMaturityYear': 2025, 
                      'maxMaturityYear': 2125, 
                      'amount': 'undefined', 
                      'userTriggered': True}
