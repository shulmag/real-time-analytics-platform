...
from auxiliary_variables import DIRECTORY, FEATURES_FOR_OUTPUT_CSV, QUANTITY, TRADE_TYPE, USERNAME, LOGGING_PRECISION
from auxiliary_functions import run_multiple_times_before_failing, response_from_individual_pricing, response_from_batch_pricing, sqltodf, get_bq_client
...
def check_individual_pricing_equals_batch_pricing(cusip, trade_type=TRADE_TYPE, quantity=QUANTITY, check_price_only=False):
    ...
    if not prices_are_equal or not ytw_are_equal:    # throw assertion error with the right information
...
@run_multiple_times_before_failing
def test_950885SN4():
    '''Tests that individual pricing and batch pricing return the same price for 950885SN4.'''
    check_individual_pricing_equals_batch_pricing('950885SN4')


@run_multiple_times_before_failing
def test_914811NS9():
    '''Tests that individual pricing and batch pricing return the same price for 914811NS9.'''
    check_individual_pricing_equals_batch_pricing('914811NS9')


## The below test has been commented out since the CUSIP is maturing very soon
# @run_multiple_times_before_failing
# def test_671409G87():
#     '''Tests that individual pricing and batch pricing return the same price for 671409G87, which 
#     sometimes has negative yields in the history and so the dollar price model may be used.'''
#     check_individual_pricing_equals_batch_pricing('671409G87', check_price_only=True)


@run_multiple_times_before_failing
def test_880461ES3():
    '''Tests that individual pricing and batch pricing return the same price for 880461ES3, which 
    is a PAC bond and so the dollar price model will be used.'''
    check_individual_pricing_equals_batch_pricing('880461ES3', check_price_only=True)


@run_multiple_times_before_failing
def test_71910EAM1():
    '''Tests that individual pricing and batch pricing return the same price for 71910EAM1, which 
    sometimes has negative yields in the history and so the dollar price model may be used.'''
    check_individual_pricing_equals_batch_pricing('71910EAM1', check_price_only=True)
