...
def test_logging_individual():
    ...
    assert error_for_successful_cusip == False, f'When pricing CUSIP {successful_cusip}, the error should be False, but is {error_for_successful_cusip}'    # must do `==` instead of `is` for boolean check because these are `np.bool_` values and not `boolean`
    assert error_for_failed_cusip == True, f'When pricing CUSIP {failed_cusip}, the error should be True, but is {error_for_failed_cusip}'    # must do `==` instead of `is` for boolean check because these are `np.bool_` values and not `boolean`
