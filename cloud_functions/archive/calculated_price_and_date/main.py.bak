...
def get_price(cusip, 
    ...
    if frequency == 0:
        ...
        assert num_of_days_in_period == round(num_of_days_in_period), f'num_of_days_in_period: {num_of_days_in_period}, frequency: {frequency}, cusip: {cusip}'
...
def compute_price(trade, yield_rate=None):
    ...
    if calc_date == trade.next_call_date: calc_date_selection = 0
    elif calc_date == trade.par_call_date: calc_date_selection = 1
    elif calc_date == trade.maturity_date: calc_date_selection = 2
    elif calc_date == trade.refund_date: calc_date_selection = 3
    else: calc_date_selection = 4
...
def get_trade_data(bqclient, publish_date):
    ...
    SELECT
        ...
        msrb_valid_from_date,
          CASE
                WHEN conduit_obligor_name IS NOT NULL THEN conduit_obligor_name
                  ELSE
...
def getSchema():
    schema = [  bigquery.SchemaField('rtrs_control_number', 'INTEGER'),
                ...
                bigquery.SchemaField("obligor_id", "STRING")
            ]
    return schema
...
def uploadData(vanilla):
    ...
    useful_columns = vanilla[['rtrs_control_number', 
                              'trade_datetime', 
                              'cusip', 
                              'calc_price', 
                              'price_to_next_call', 
...
# selects the latest reference data for a given cusip
query_for_redis = '''
SELECT
    coupon,
    cusip,
...
where date_diff( current_datetime("America/New_York"),recent[safe_offset(0)].msrb_valid_from_date, minute) < 60'''
...
def main(args):
    ...
    if df.empty:
        ...
        return f"Success! Function took {overall_delta}" 