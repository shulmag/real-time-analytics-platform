...
from automated_training_auxiliary_functions import EMAIL_RECIPIENTS, send_email, TESTING
...
def send_training_log(attachment_path, recipients:list, model:str):
    ...
    msg = MIMEMultipart()
    if TESTING:
        msg['Subject'] = f'(TESTING) Logs for model-training-component {datetime.now().strftime("%Y-%m-%d")}'
    else: 
        msg['Subject'] = f'Logs for model-training-component'
