...
REFERENCE_DATA_REDIS_CLIENT = redis.Redis(host='10.2.196.133', port=6379, db=0)
...
MAX_NUM_DAYS_FOR_REFERENCE_DATA_POINT_IN_TIME = 390    # number of days for which if there are more than reference data updates for a particular CUSIP; this value should be the same as `MAX_NUM_DAYS_FOR_TRADES_IN_HISTORY` in `cloud_functions/fast_trade_history_redis_update/main.py`

# feature names must be identical to `modules/auxiliary_variables.py::REFERENCE_DATA_FEATURES`
REFERENCE_DATA_FEATURES = ['current_coupon_rate AS coupon',    # `coupon` is represented with `current_coupon_rate` in `eng-reactor-287421.reference_data_v1.reference_data_flat`, but this needs to match the schema of `eng-reactor-287421.auxiliary_views.trade_history_latest_ref_data_minimal_exclusions`
                           'cusip',
                           'ref_valid_from_date',
                           'ref_valid_to_date',
                           ...
                           # 'most_recent_event',
                           # 'event_exists',
                           'issue_key AS series_id',    # `series_id` is represented with `issue_key` in `eng-reactor-287421.reference_data_v1.reference_data_flat`, but this needs to match the schema of `eng-reactor-287421.auxiliary_views.trade_history_latest_ref_data_minimal_exclusions`
                           'CONCAT(IFNULL(organization_primary_name, ""), " ", IFNULL(instrument_primary_name, ""), " ", IFNULL(conduit_obligor_name, "")) AS security_description',    # `security_description` needs to match the schema of `eng-reactor-287421.auxiliary_views.trade_history_latest_ref_data_minimal_exclusions`
...
def get_new_reference_data_query() -> str:
    ...
    `date(ice_file_date) = current_date("America/New_York") AND ref_valid_to_date > current_datetime("America/New_York")` 
    will retrieve reference data from an update that occurred today (hence, `date(ice_file_date) = current_date("America/New_York")`) 
    and data that is most current (hence, `ref_valid_to_date > current_datetime("America/New_York")`). The `ORDER BY cusip` 
    ...
    return f'''SELECT {", ".join(REFERENCE_DATA_FEATURES)} 
               ...
               WHERE cusip IS NOT NULL AND DATE(ice_file_date) = CURRENT_DATE("America/New_York") AND DATETIME(ref_valid_to_date) > CURRENT_DATETIME("America/New_York")
               ORDER BY cusip'''
...
def get_data_from_pickle_file_if_query_matches(query: str, file_name: str) -> pd.DataFrame:
    ...
    assert TESTING, f'Should not be using pickled files if not testing'
...
def upload_data_to_redis(cusip: str, reference_data: np.array, valid_from_date: str) -> deque:
    if TESTING: python_logging.debug(f'Uploading data to redis for CUSIP: {cusip}')
    if valid_from_date != FIRST_RECORD_VALID_FROM_DATE and REFERENCE_DATA_REDIS_CLIENT.exists(cusip):
        reference_data_deque = pickle.loads(REFERENCE_DATA_REDIS_CLIENT.get(cusip))
        ...
        # iterate backwards through the deque and remove reference data series if the reference data valid to date is older than MAX_NUM_DAYS_FOR_REFERENCE_DATA_POINT_IN_TIME in the past from the current date; iterating backwards since reference data numpy arrays are in descending order of `ref_valid_from_date`
        ...
        if len(reference_data_deque) > 0:
            ...
            if most_recent_reference_data[pos_ref_valid_from_date] != reference_data_ref_valid_from_date:    # if the `ref_valid_from_date` for most recent reference data is the same as that of the proposed update (`reference_data`), then these snapshots are identical and the update should not happen (i.e., this is not a "new" snapshot)
                most_recent_reference_data[pos_ref_valid_to_date] = reference_data_ref_valid_from_date    # update the `ref_valid_to_date` of the most recent item in the deque when updating the deque because it will be set to a datetime far in the future and needs to be set to `ref_valid_from_date` for the new update
        ...
        if valid_from_date == FIRST_RECORD_VALID_FROM_DATE:
            ...
            python_logging.debug(f'No previous reference data exists for {cusip}')
        reference_data_deque = deque([reference_data])

    if TESTING:
        python_logging.debug(reference_data_deque)
    else:
        REFERENCE_DATA_REDIS_CLIENT.set(cusip, pickle.dumps(reference_data_deque))
    return reference_data_deque    # unused return value that can be used for debugging


@function_timer
def update_reference_data_redis(new_reference_data: pd.DataFrame) -> None:
    cusips = new_reference_data['cusip'].tolist()    # due to the `ORDER BY ` clause in the query, `new_reference_data` should be sorted in alphabetical order by CUSIP
    ...
    for _, row in new_reference_data.iterrows():    # TODO: use parallelization if `len(cusips)` is large
        cusip, valid_from_date = row['cusip'], row['ref_valid_from_date']
        upload_data_to_redis(cusip, row.to_numpy(), valid_from_date)    # use `.to_numpy()` to reduce the space consumption of `row` by 95%
