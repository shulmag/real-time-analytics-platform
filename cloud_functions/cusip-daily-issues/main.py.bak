...
def cusip_daily_issues(args):
    ...
    for i in range(4):
        ...
        try:
            ...
            if data.empty:
                python_logging.warning("CUSIP Daily Issues: Empty Data", f'The processed data is empty for {new_date_string}. Please check the input file.')    # f"<html><body><p>The processed data is empty for {new_date_string}. Please check the input file.</p></body></html>")
                continue
            ...
            if last_entry_date is None or new_date > last_entry_date:
                ...
                if result == "SUCCESS":
                    ...
                    print("CUSIP Daily Issues: Success", message)    # f"<html><body><p>{message}</p></body></html>")
                else:
                    message = f"Data upload failed for {new_date_string}."
                    python_logging.warning("CUSIP Daily Issues: Failure", message)    # f"<html><body><p>{message}</p></body></html>")
            else:
