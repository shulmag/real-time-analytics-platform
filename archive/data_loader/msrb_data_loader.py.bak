#schema: http://msrb.org/Market-Transparency/Subscription-Services-and-Products/MSRB-Transaction-Subscription/Historical-Data-Format.aspx
...
def trade_message_to_full_db_record(trade_message): 
    names = ['rtrs_control_number',	'trade_type',	'cusip',	'security_description',	'dated_date',	'coupon',	'maturity_date',	'is_trade_before_issue',	'assumed_settlement_date',	'trade_date',	'time_of_trade',	'settlement_date',	'par_traded',	'dollar_price',	'yield',	'brokers_broker',	'is_weighted_average_price',	'list_offering_price_or_takedown_indicator',	'publish_date',	'publish_time',	'version',	'unable_to_verify_dollar_price',	'is_alternative_trading_system',	'is_non_transaction_based_compensation',	'is_trade_with_a_par_amount_over_5MM']
    parsed_messages = trade_message.split(',')
    dict_record_with_headers = dict(zip(names,parsed_messages))
...
def add_to_bigquery(rows_to_insert):
    table_id = "eng-reactor-287421.MSRB.msrb_trades_2019_2020"
    #Create schema
    ...
    for dir in os.listdir(path):
        if not dir.startswith('.'):
            for filename in os.listdir(os.path.join(path,dir)):
                ...
                with open(os.path.join(path,dir,filename), 'r') as f:
                    data = f.read()
