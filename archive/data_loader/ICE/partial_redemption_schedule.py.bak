...
dataset = 'FULL_ICE'
table = 'partial_redemption_schedule'
...
def partial_redemption_schedule(new_elem,final_list):
    ...
    if len(list_of_elem)>0:
        for element in list_of_elem[0].getchildren():
            if element.tag == "partial_redemption":
                new_dict = {}
...
def create_df_partial_redemption_schedule(new_elem,final_list):
#     new_dict = {}
    
    partial_redemption_schedule(new_elem,final_list)
...
def new_fast_iter(context,data_timestamp, *args, **kwargs):
    ...
    for _,elem in context:
        new_elem = etree.parse(BytesIO(etree.tostring(elem)))
        create_df_partial_redemption_schedule(new_elem,final_list)
