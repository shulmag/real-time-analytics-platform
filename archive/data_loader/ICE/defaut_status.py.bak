...
def default_status(new_elem,final_list):
    
    list_of_elem = new_elem.xpath("//instrument/global_information/instrument_details")
...
def create_df_default_status(new_elem,final_list):
#     new_dict = {}
    
    default_status(new_elem,final_list)
...
def new_fast_iter(context,data_timestamp, *args, **kwargs):
    ...
    for _,elem in context:
        new_elem = etree.parse(BytesIO(etree.tostring(elem)))
        create_df_default_status(new_elem,final_list)
