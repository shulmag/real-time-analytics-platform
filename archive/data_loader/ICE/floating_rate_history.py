...
def floating_rate_history(new_elem,final_list):
    ...
    if len(list_of_elem)>0:
        for element in list_of_elem[0].getchildren():
            if element.tag == "floating_rate_history":
                new_dict = {}
...
def create_df_floating_rate_history(new_elem,final_list):

    floating_rate_history(new_elem,final_list)
...
def new_fast_iter(context,data_timestamp, *args, **kwargs):
    ...
    for _,elem in context:
        new_elem = etree.parse(BytesIO(etree.tostring(elem)))
        create_df_floating_rate_history(new_elem,final_list)
