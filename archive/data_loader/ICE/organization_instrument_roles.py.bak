...
dataset = 'FULL_ICE'
table = 'organization_instrument_roles'
...
def organization_instrument_roles(new_elem,final_list):
    
    list_of_elem = new_elem.xpath("//instrument/global_information/relations/instrument_roles")
    ...
    if len(list_of_elem)>0:
        for element in list_of_elem[0].getchildren():
            if element.tag == "role":
                ...
                new_dict.update({"type_code":element.text})

                final_list.append(new_dict)

def create_df_organization_instrument_roles(new_elem,final_list):    
    organization_instrument_roles(new_elem,final_list)
...
def new_fast_iter(context,data_timestamp, *args, **kwargs):
    ...
    for _,elem in context:
        new_elem = etree.parse(BytesIO(etree.tostring(elem)))
        create_df_organization_instrument_roles(new_elem,final_list)     
        count += 1
