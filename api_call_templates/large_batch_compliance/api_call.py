'''
Description: This script performs large batch pricing and compiles a CSV with the results. This script was last used for creating compliance results for Edward Jones in 2024-11.
'''
import csv
import time
import itertools
...
from auxiliary_variables import USERNAME, PASSWORD, CSV_FILEPATH, EASTERN, UNIQUE_QUANTITIES, UNIQUE_TRADE_TYPES, MAX_NUMBER_OF_CUSIPS_PER_BATCH, COLUMNS_TO_KEEP
    ...
    def wrapper(*args, **kwargs):    # using the same formatting from https://docs.python.org/3/library/functools.html
        ...
        while runs_so_far < max(max_runs_for_runtime_error, max_runs_for_http_error):
            ...
            if runs_so_far >= max_runs:
                ...
                raise exception
            print(f'WARNING: Caught {type(exception)}: {exception}, and will retry {function.__name__} {max_runs - runs_so_far} more times (next run will be {sleep_time} seconds later)')
...
def get_cusips_quantities_tradetypes_from_csv(csv_filename):
    ...
    cusips, quantities, trade_types, user_prices, trade_datetimes  = [], [], [], [], []
    with open(csv_filename, mode='r') as file:
        ...
        for row in csv_reader:
            ...
            if len(row) > 3:
                user_prices.append(row[3])
            if len(row) > 4:
                trade_datetimes.append(row[4])
    assert (len(cusips) == len(quantities) == len(trade_types) == len(user_prices) == len(trade_datetimes)) or (len(quantities) == len(trade_types) == len(user_prices) == len(trade_datetimes) == 0), 'Currently support a CSV with the following format: (1) every row is a unique CUSIP, or (2) every row has a CUSIP, quantity, trade type, user price, and trade datetime'
    print(f'Extracted {len(cusips)} CUSIPs from {csv_filename}')
    return cusips, quantities, trade_types, user_prices, trade_datetimes
...
def exponential_backoff(url, data, max_retries: int = 5, backoff_factor: int = 2):
    ...
    while retries < max_retries:
        try:
            ...
            print(f'WARNING: Request at {url} timed out with {type(e)}: {e}. Retrying after {wait_time} seconds. Request called with data:\n{data}')
...
def call_batch_pricing(cusip_list: list, quantity_list: list, trade_type_list: list, user_price_list: list, trade_datetime_list: list, username: str, password: str) -> pd.DataFrame:
...
def price_batches(cusip_list_batches, quantity_list_batches, trade_type_list_batches, user_price_list_batches, trade_datetime_list_batches, username: str, password: str) -> pd.DataFrame:
    call_batch_pricing_func = lambda cusip_list, quantity_list, trade_type_list, user_price_list, trade_datetime_list : call_batch_pricing(cusip_list, quantity_list, trade_type_list, user_price_list, trade_datetime_list, username, password)
    cusip_quantity_trade_type_batches = zip(cusip_list_batches, quantity_list_batches, trade_type_list_batches, user_price_list_batches,trade_datetime_list_batches)
    priced_batches = [call_batch_pricing_func(cusip_list_batch, quantity_list_batch, trade_type_list_batch, user_price_list_batch, trade_datetime_list_batch) for cusip_list_batch, quantity_list_batch, trade_type_list_batch, user_price_list_batch, trade_datetime_list_batch in cusip_quantity_trade_type_batches]
    return priced_batches


@function_timer
def call_price_batches(username: str, password: str, cusip_list_batches: list, quantity_list_batches: list, trade_type_list_batches: list, user_price_list_batches: list, trade_datetime_list_batches: list) -> list:
    num_batches = len(cusip_list_batches)
    print(f'Total number of batches: {num_batches}')
    max_async_calls_at_once = 50    # 200
    ...
    last_cusip_list, last_quantity_list, last_trade_type_list, last_user_price_list, last_trade_datetime_list = cusip_list_batches.pop(), quantity_list_batches.pop(), trade_type_list_batches.pop(), user_price_list_batches.pop(), trade_datetime_list_batches.pop()
    last_batch_priced = price_batches([last_cusip_list], [last_quantity_list], [last_trade_type_list], [last_user_price_list], [last_trade_datetime_list], username, password)
    
    priced_batches = []
    for batch_group_start_idx in range(0, len(cusip_list_batches), max_async_calls_at_once): 
        priced_batches.extend(asyncio.run(price_batches_async(
            cusip_list_batches[batch_group_start_idx : batch_group_start_idx + max_async_calls_at_once], 
            ...
            user_price_list_batches[batch_group_start_idx : batch_group_start_idx + max_async_calls_at_once],
            trade_datetime_list_batches[batch_group_start_idx : batch_group_start_idx + max_async_calls_at_once], 
            username, 
            password)))
        print(f"Now at: {batch_group_start_idx}/{len(cusip_list_batches)}")
    priced_batches = priced_batches + last_batch_priced
    return priced_batches


if __name__ == '__main__':
    current_datetime = datetime.now(EASTERN).strftime('%Y-%m-%d_%H-%M-%S')
    cusip_list, quantity_list, trade_type_list, user_price_list,trade_datetime_list = get_cusips_quantities_tradetypes_from_csv(CSV_FILEPATH)
    if len(quantity_list) == 0: cusip_list, quantity_list, trade_type_list, user_price_list,trade_datetime_list = get_cusip_list_and_quantity_list_and_trade_type_list(cusip_list, UNIQUE_QUANTITIES, UNIQUE_TRADE_TYPES)
    cusip_list, quantity_list, trade_type_list, user_price_list, trade_datetime_list = list(cusip_list), list(quantity_list), list(trade_type_list), list(user_price_list),list(trade_datetime_list)    # need to convert each of these to a list so that we can mutate them using `.pop(...)`

    last_cusip, last_quantity, last_trade_type, last_user_price,last_trade_datetime = cusip_list.pop(), quantity_list.pop(), trade_type_list.pop(), user_price_list.pop(), trade_datetime_list.pop()    # isolate only the last line item to be the one that is priced individually before making the asynchronous calls to create a speedup
    ...
    user_price_list_batches = split_list_into_batches(user_price_list, MAX_NUMBER_OF_CUSIPS_PER_BATCH) + [[last_user_price]]
    trade_datetime_list_batches = split_list_into_batches(trade_datetime_list, MAX_NUMBER_OF_CUSIPS_PER_BATCH) + [[last_trade_datetime]]

    priced_batches = call_price_batches(USERNAME, PASSWORD, cusip_list_batches, quantity_list_batches, trade_type_list_batches, user_price_list_batches, trade_datetime_list_batches)
    ...
    priced_csv_filename = f'results_{current_datetime}.csv'
