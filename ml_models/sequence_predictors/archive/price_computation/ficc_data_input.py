...
'''
The trade_dict_to_list converts the recent trades dictionary to a list.
...
Dividing the trade size by 1000 to reduce the absolute scale. 
Taking the log of the number of seconds between the historical trade and the latest trade
'''

def trade_dict_to_list(trade_dict):
...
def trade_list_to_array(trade_history_input):
    if len(trade_history_input) == 0:
        return np.array([])

    trades_list = []
    for entry in trade_history_input:
...
def pocess_trade_history_input(df,SEQUENCE_LENGTH, NUM_FEATURES):
    ...
    df['trade_history_input'] = df.recent.apply(trade_list_to_array)
