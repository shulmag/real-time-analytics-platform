...
print(PREDICTORS)
#Ensures tensorflow does not allocate the entire GPU memory to the current kernel. Unimportant here but allows multiple notebooks to use the GPU in VM.
...
def get_args():
    ...
    parser.add_argument('--train_start', help='Start of training data', default = '2023-06-01', type=str)
    parser.add_argument('--train_end', help='End of training data', default = '2023-06-04', type=str)
    parser.add_argument('--test_start', help='Start of test data', default = '2023-06-04', type=str)
    parser.add_argument('--test_end', help='End of test data', default = '2023-06-07', type=str) 
    ...
    for variable in BINARY + CATEGORICAL_FEATURES + NON_CAT_FEATURES:
        # parser.add_argument('--DROP_'+variable, action='store_true', help=f'Option to drop variable [{variable}]')
        parser.add_argument('--DROP_'+variable, type=bool, default = False, help=f'True/false option to drop variable [{variable}]')
...
if __name__=="__main__":
    ...
    #parse arguments for experiment run 
    args = get_args()
    timestamp = datetime.now().strftime('%Y-%m-%d-%H:%M')
    
    train_start = args.get('train_start', '2023-06-01')
    train_end = args.get('train_end', '2023-06-04')
    test_start = args.get('test_start', '2023-06-04')
    test_end = args.get('test_end', '2023-06-07')
    ...
    for variable in BINARY + CATEGORICAL_FEATURES + NON_CAT_FEATURES:
        if args['DROP_'+variable]:
            for l in [BINARY, CATEGORICAL_FEATURES, NON_CAT_FEATURES]:
                if variable in l:
                    l.remove(variable)
                    if variable in PREDICTORS:
                        PREDICTORS.remove(variable)
                    print(f'Removing {variable} from features.')
    
    
    #load encoders from same file as production
    with fs.open('gs://automated_training/encoders.pkl') as f:
        encoders = pickle.load(f)
    fmax = {key: len(value.classes_) for key, value in encoders.items()}
    
    #data processing is done beforehand to avoid repeat computation unecessary CPU resources being idle during model training 
    processed_data = load_data_from_pickle(file, bucket = bucket)    
    print(f'Min date: {processed_data.trade_date.min()}\nMax date: {processed_data.trade_date.max()}')
    ...
    #july results were showing anomolous predictions 
    pred_filter = (-5000 < test_dataframe['prediction']) & (test_dataframe['prediction'] < 5000)
    if len(test_dataframe) - sum(pred_filter):
        print(f'There are {len(test_dataframe) - sum(pred_filter)} trades with extreme outlier predictions. Removing them for MAE calculation. Check saved predictions for those specific trades.')
    
    test_dataframe = test_dataframe[pred_filter]
    mae = mean_absolute_error(test_dataframe['prediction'], test_dataframe[TARGET])
    ...
    model_path = f'gs://custom-train-job-test/models/model_{timestamp}'
    model.save(model_path)
    print(f'Model saved to {model_path}.')
    docs_path = model_path+'_args'
    with fs.open(docs_path, 'wb') as gf:
        pickle.dump(args, gf)
        print(f'Model args saved to {docs_path}.')
    
    predictions_path = f'gs://custom-train-job-test/predictions/predictions_{timestamp}'
    test_dataframe[['rtrs_control_number', 'prediction']].to_pickle(predictions_path)
