    ...
    try:          
        ...
        for trade_message in trade_messages:
            ...
            if processed_message: 
                ...
                data = next(iter(processed_message.values()))
                if data['transaction_type'] == 'C':
                    cancellations.append(next(iter(processed_message.keys())))
                else:
...
def update_intraday_cusips(processed_trade_messages, cancellations, trades_df):
    ...
    if cancellations: print(f'Trades with rtrs {cancellations} were cancelled and removed from trades_df')
