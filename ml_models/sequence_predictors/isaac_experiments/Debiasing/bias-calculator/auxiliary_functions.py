...
def simulate_weighted_average(df, weighting_col, error_col, window_size = 2000, weighting_method = 'default', mask_large = 35):
...
def debias_series(pred, truth, bias_correction):
    ...
    original_MAE = mean_absolute_error(pred, truth)
    corrected_MAE = mean_absolute_error(corrected_pred, truth)
    
    print(f'Original bias: {np.mean(pred-truth):.3f}, Original MAE: {original_MAE:.3f}, Corrected bias: {np.mean(corrected_pred-truth):.3f}, Corrected MAE: {corrected_MAE:.3f}')
...
def getSchema():
    ...
    schema = [
        bigquery.SchemaField("Timestamp","DATETIME", "NULLABLE"),
        bigquery.SchemaField("MAE_all_trades","FLOAT", "NULLABLE"),
        bigquery.SchemaField("MAE_new_ys_trades","FLOAT", "NULLABLE"),
        bigquery.SchemaField("MAE_all_trades_debiased","FLOAT", "NULLABLE"),
        bigquery.SchemaField("MAE_new_ys_trades_debiased","FLOAT", "NULLABLE")]
...
def load_from_cloud_storage(path, fs):
    try:
        ...
        print(f'{path} was not found in GCS. Exception {f}.')
...
def upload_to_cloud_storage(path, obj, fs):
    try:
        ...
        print(f'{path} upload to GCS was not successful. Exception {f}.')