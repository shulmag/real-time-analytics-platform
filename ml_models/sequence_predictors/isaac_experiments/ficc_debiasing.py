...
def calculate_weighted_average(data, weighting_col, error_col, method = 'default', mask_large = 35):
    ...
    if method == 'mask_large':
        #this gives larger weight to small errors but sets also moderates how much extremely small errors can be weighted 
        weights = np.where(np.abs(weights) <= mask_large, weights, 0) 
...
def simulate_weighted_average(df, weighting_col, error_col, groupby_cols = ['trade_date'], window_size = 2000, weighting_method = 'default', mask_large = 35):
    ...
    for sub_df in groupby_dfs:
        biases.append(calculate_weighted_average(sub_df, weighting_col, error_col, method = weighting_method, mask_large = 35))
...
def calculate_weighted_average_masked(data, weighting_col, error_col, method = 'default', mask_large = 35, seconds_ago_mask = 60):
...
def simulate_weighted_average_masked(df, weighting_col, error_col, groupby_cols = ['trade_date'], window_size = 2000, weighting_method = 'default', mask_large = 35):
    '''Simulates debiasing procedure in production by calculating rolling average bias 
    ...
    #if we are weigthing by the error column then don't slice the column twice
    subset = [weighting_col, error_col, 'first_published_datetime']
    if weighting_col == error_col:  subset.remove(error_col)
    ...
    biases = []
    for sub_df in groupby_dfs:
        biases.append(calculate_weighted_average_masked(sub_df, weighting_col, error_col, method = weighting_method, mask_large = 35))
...
def analyze_debiasing_MAE(df, bias_correction, date = None, prediction_col = 'prediction', target_col='new_ys'):
    ...
    summary = df[['trade_datetime','trade_date', prediction_col, target_col]].set_index('trade_date')
...
def analyze_debiasing_day(date, df, bias_correction, prediction_col = 'prediction', target_col='new_ys', first_N_trades = 500, last_N_trades = 200):
