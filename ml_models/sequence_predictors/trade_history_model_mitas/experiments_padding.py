...
from ficc.utils.trade_dict_to_list import TRADE_TYPE_MAPPING, FEATURES_TO_INDEX_IN_HISTORY, quantity_diff
...
# torch.multiprocessing.set_sharing_strategy('file_system')    # in response to this error: RuntimeError: Too many open files. Communication with the workers is no longer possible. Please increase the limit using `ulimit -n` in the shell or change the sharing strategy by calling `torch.multiprocessing.set_sharing_strategy('file_system')` at the beginning of your code
...
train_data_only_reference, test_data_only_reference = get_train_test_data_trade_datetime(trade_data, DATE_TO_SPLIT)
assert len(train_data_only_reference) != 0 and len(test_data_only_reference) != 0, 'Either train or test data is empty. Consider checking how the train test split is being performed.'
columns_to_remove = DATA_PROCESSING_FEATURES + TRADE_HISTORY + IDENTIFIERS
train_data_only_reference = train_data_only_reference.drop(columns=columns_to_remove)
test_data_only_reference = test_data_only_reference.drop(columns=columns_to_remove)
print(f'Number of trades for training: {len(train_data_only_reference)}.\
    Number of trades for testing: {len(test_data_only_reference)}')
...
BATCH_SIZE = 1000
NUM_WORKERS = 0    # lowering the number of workers due to OSError: [Errno 12] Cannot allocate memory: https://github.com/prlz77/ResNeXt.pytorch/issues/5; 8 if is_gpu_available() or is_mps_available() else 0
NUM_EPOCHS = 100
...
# TODO: uncomment the calc_day_cat feature when using the new data file
...
get_neighbor_feature_dict = {feature: get_neighbor_feature(feature) for feature in REFERENCE_FEATURES_TO_ADD}
APPENDED_FEATURES_FUNCTIONS = {**APPENDED_FEATURES_FUNCTIONS, **get_neighbor_feature_dict}    # combine two dictionaries together for Python v3.5+: https://stackoverflow.com/questions/38987/how-do-i-merge-two-dictionaries-in-a-single-expression
...
assert list(FEATURES_TO_INDEX_IN_HISTORY.keys()) == list(APPENDED_FEATURES_FUNCTIONS.keys())    # insertion order of the dictionary is preserved for Python v3.7+ so this will check if the ordering of the keys are the same
...
for name, (categories_to_match, filtering_conditions) in tqdm(related_trades_criterion.items()):
    ...
    if os.path.exists(filepath):    # check if a file exists https://www.pythontutorial.net/python-basics/python-check-if-file-exists/
        ...
        trade_data_flattened_trade_history_and_related_trades[name].to_pickle(filepath, protocol=4)    # https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.to_pickle.html
...
related_trades_criterion_opt = list(related_trades_criterion.keys())[0]    # optimal name is the one with the minimum test error
...
embed_with_embedding_dicts = lambda df: embed_with_dicts(df, embedding_dicts)


trade_data_to_create_trade_history = trade_data
if ENCODE_REFERENCE_FEATURES: trade_data_to_create_trade_history = encode_with_label_encoders(trade_data_to_create_trade_history)
trade_data[TRADE_HISTORY[0]] = add_reference_data_to_trade_history(trade_data_to_create_trade_history, REFERENCE_FEATURES_TO_ADD, TRADE_HISTORY)
...
# Replace features with encoded values
if ENCODE_REFERENCE_FEATURES:
    ...
    for feature in REFERENCE_FEATURES_TO_ADD:
        ...
        for past_trade_idx in range(NUM_TRADES_IN_RELATED_TRADE_HISTORY):
            feature_name = get_appended_feature_name(past_trade_idx, feature, appended_feature_prefix)
            trade_data_flattened_trade_history_and_related_trades[feature_name] = encoder.transform(trade_data_flattened_trade_history_and_related_trades[feature_name].to_numpy())    # transform the categorical feature column to its encoding


trade_data[TRADE_HISTORY_RELATED[0]] = feature_group_as_single_feature(trade_data_flattened_trade_history_and_related_trades, appended_features_related_trades, NUM_TRADES_IN_RELATED_TRADE_HISTORY)
...
for trade_history_column in TRADE_HISTORY + TRADE_HISTORY_RELATED:
    ...
    assert np.issubdtype(trade_history_column_dtype, np.number)    # asserts that the dtype of the trade history array is a numerical type: https://stackoverflow.com/questions/29518923/numpy-asarray-how-to-check-up-that-its-result-dtype-is-numeric
...
rnn_name = f'{RECURRENT_ARCHITECTURE}_{num_layers}_layers_{hidden_size}_hidden_size_{NUM_EPOCHS}_epochs'
...
# format for this dictionary: key is the name of the feature; value is a tuple where the first item is a function of the current trade and related trade, and the second item is the default value to be filled in if that value does not exist
APPENDED_FEATURES_FUNCTIONS_AND_NONZERO_PADDING_DEFAULT_VALUES = {key: (function, DEFAULT_VALUES_NONZERO_PADDING[key]) for key, function in APPENDED_FEATURES_FUNCTIONS.items()}
...
if os.path.exists(filepath):    # check if a file exists https://www.pythontutorial.net/python-basics/python-check-if-file-exists/
    ...
    trade_data_nonzero_padding_related_trades = append_recent_trade_data(trade_data_nonzero_padding, 
                                                                         NUM_TRADES_IN_RELATED_TRADE_HISTORY, 
                                                                         APPENDED_FEATURES_FUNCTIONS_AND_NONZERO_PADDING_DEFAULT_VALUES, 
                                                                         feature_prefix=appended_feature_prefix, 
...
trade_data_nonzero_padding[TRADE_HISTORY_RELATED[0]] = feature_group_as_single_feature(trade_data_nonzero_padding_related_trades, 
                                                                                       appended_features_related_trades, 
                                                                                       NUM_TRADES_IN_RELATED_TRADE_HISTORY)
...
for seed in range(len(test_l1_losses), NUM_SEEDS):
    ...
    _, (_, _, (test_l1_loss, _)) = same_rnn_caller(train_data_both_histories_8_16, 
                                                   test_data_both_histories_8_16, 
                                                   True)
    ...
    _, (_, _, (test_l1_loss, _)) = same_rnn_caller(train_data_nonzero_padding_both_histories_8_16, 
                                                   test_data_nonzero_padding_both_histories_8_16, 
                                                   True)
...
print(f'Average loss (nonzero padding): {sum(test_l1_losses_nonzero_padding) / NUM_SEEDS}\t\tAll losses: {test_l1_losses_nonzero_padding}')