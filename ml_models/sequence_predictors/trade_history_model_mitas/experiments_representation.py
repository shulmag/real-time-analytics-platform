...
if os.path.exists(embeddings_arrays_filepath):    # check if a file exists https://www.pythontutorial.net/python-basics/python-check-if-file-exists/
    ...
    with open(embeddings_arrays_filepath, 'wb') as pickle_handle: pickle.dump(embeddings_arrays, pickle_handle, protocol=4)    # use template from https://stackoverflow.com/questions/11218477/how-can-i-use-pickle-to-save-a-dict-or-any-other-python-object
...
#     one_hot_encoded_reference_data[TRADE_HISTORY[0]] = add_reference_data_to_trade_history(trade_data_one_hot_encoded, REFERENCE_FEATURES_TO_ADD, TRADE_HISTORY)
...
#     one_hot_encoded_reference_data[TRADE_HISTORY_RELATED[0]] = feature_group_as_single_feature(trade_data_flattened_trade_history_and_related_trades_one_hot_encoded, appended_features_related_trades, NUM_TRADES_IN_RELATED_TRADE_HISTORY, flatten_each_row=True, multiprocessing=True)
...
if os.path.exists(embedded_reference_data_filepath):    # check if a file exists https://www.pythontutorial.net/python-basics/python-check-if-file-exists/
    ...
    embedded_reference_data[TRADE_HISTORY[0]] = add_reference_data_to_trade_history(trade_data_embedded, REFERENCE_FEATURES_TO_ADD, TRADE_HISTORY, True)
    ...
    embedded_reference_data[TRADE_HISTORY_RELATED[0]] = feature_group_as_single_feature(trade_data_flattened_trade_history_and_related_trades_embedded, appended_features_related_trades, NUM_TRADES_IN_RELATED_TRADE_HISTORY, flatten_each_row=True, multiprocessing=True)
...
if os.path.exists(reference_data_representation_losses_filepath):
    print(f'Loading losses from {reference_data_representation_losses_filepath}')
...
for name, trade_data_both_histories in trade_history_data.items():
    if name not in reference_data_representation_losses:
        ...
        _, test_l1_loss = same_rnn_caller(limit_history_8_same_cusip_16_related_caller(train_data_both_histories), 
...
reference_data_representation_losses_ascending_order = sorted(reference_data_representation_losses, key=lambda name: reference_data_representation_losses.get(name))    # sort by minimum test error
